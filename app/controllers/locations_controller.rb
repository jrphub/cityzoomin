class LocationsController < ApplicationController
  ...
  def show
    if (params[:city])
      ...
      user_id, locations.name as locname, city, state, country, username, email").where("city=?",params[:city])
      ...
      .joins('INNER JOIN locations ON microposts.location_id = locations.id')
      .select("microposts.id,content, location_id, title, category,microposts.created_at, microposts.updated_at, 
      user_id, locations.name as locname, city, state, country, username, email").where("state=?",params[:state])
      .paginate(page: params[:page],per_page:3)
      ...
      .joins('INNER JOIN locations ON microposts.location_id = locations.id')
      .select("microposts.id,content, location_id, title, category,microposts.created_at, microposts.updated_at, 
      user_id, locations.name as locname, city, state, country, username, email").where("country=?",params[:country])
      .paginate(page: params[:page],per_page:3)
