class LocationsController < ApplicationController
  ...
  def show
    if (params[:city])
      ...
      user_id, name, city, state, country, username, email, has_photo").where("city=?",params[:city])
      ...
      .joins('INNER JOIN locations ON microposts.location_id = locations.id')
      .select("microposts.id,content,has_photo, location_id, title, category,microposts.created_at, microposts.updated_at, 
      user_id,name, city, state, country, username, email, has_photo").where("state=?",params[:state])
      .paginate(page: params[:page],per_page:3)
      ...
      .joins('INNER JOIN locations ON microposts.location_id = locations.id')
      .select("microposts.id,content,has_photo, location_id, title, category,microposts.created_at, microposts.updated_at, 
      user_id,name, city, state, country, username, email, has_photo").where("country=?",params[:country])
      .paginate(page: params[:page],per_page:3)
      ...
      .joins('INNER JOIN locations ON microposts.location_id = locations.id')
      .select("microposts.id,content,has_photo, location_id, title, category,microposts.created_at, microposts.updated_at, 
      user_id, name, city, state, country, username, email, has_photo").where(:location_id=>params[:id])
      .paginate(page: params[:page],per_page:3)
